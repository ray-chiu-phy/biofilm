  ...
  FixGrowth(lmp, narg, arg)
{
  ...
  imetab2 = grid->find(arg[3]);
  if (imetab2 < 0)
    error->all(FLERR, "Can't find substrate(metab2) name");
  metab2_affinity = utils::numeric(FLERR,arg[4],true,lmp);
...
void FixGrowthCross1::update_cells()
{
  ...
  for (int i = 0; i < grid->ncells; i++) {
    if (grid->mask[i] & GRID_MASK) {
      // cross_feeder1 growth rate based on metabolite2
      double tmp1 = growth * conc[imetab2][i] / (metab2_affinity + conc[imetab2][i]);
      // metabolite1 export-induced growth reduction
      double tmp2 = tmp1 * (-3.4897 * exp(-metab1_exp/0.048) + 3.4092);  // represents the extra sub consumption for metabolite1 production  min(meta1_exp=0.0112)
