  ...
  FixGrowth(lmp, narg, arg)
{
  ...
  imetab1 = grid->find(arg[3]);
  if (imetab1 < 0)
    error->all(FLERR, "Can't find substrate(metabolite1) name");
  metab1_affinity = utils::numeric(FLERR,arg[4],true,lmp);
...
void FixGrowthCross2::update_cells()
{
  ...
  for (int i = 0; i < grid->ncells; i++) {
    if (grid->mask[i] & GRID_MASK) {
      // cross_feeder2 growth rate based on  metabolite1
      double tmp1 = growth * conc[imetab1][i] / (metab1_affinity + conc[imetab1][i]);
...
void FixGrowthCross2::update_atoms()
{
  ...
  for (int i = 0; i < grid->ncells; i++) {
    ...
    double tmp1 = growth * conc[imetab1][i] / (metab1_affinity + conc[imetab1][i]);
    // metabolite2 export-induced growth reduction
    double tmp3 = tmp1 * (0.141 * exp(-metab2_exp/0.063) + 0.9);   
