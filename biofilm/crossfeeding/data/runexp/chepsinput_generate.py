...
- 公式：seed = i^2 * (v^2) + 2*v + offset
    ...
    eps1: v=4, offset=104
    eps2: v=5, offset=105
- 產生檔名：inputscript_CHEPS{i}.nufeb
"""
...
# ===== 可在這裡調整 =====
OUTPUT_PREFIX = "inputscript_CHEPS"  # 輸出檔名前綴
N_FILES = 50                       # 產生數量：索引 INDEX_START..INDEX_START+N_FILES-1
INDEX_START = 1                    # 起始索引
TEMPLATE_NAME = "inputscript_CHEPS.nufeb"
# 參數表（v, offset）
LABELS = {
    "div1": (1, 101),
    "div2": (2, 102),
...
EPS_PATTERN = re.compile(
    r'^(?P<prefix>\s*fix\s+eps(?P<num>\d+)\b[^\n]*?\bnufeb/eps_secretion\b[^\n]*?\s)'
    r'(?P<seed>\d+)(?P<suffix>\s*(?:#.*)?)$',
    flags=re.MULTILINE
)
