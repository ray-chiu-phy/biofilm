...
FixGrowthCross2::FixGrowthCross2(LAMMPS *lmp, int narg, char **arg) :
  FixGrowth(lmp, narg, arg)
{
  ...
  if (narg < 8)
    error->all(FLERR, "Illegal fix nufeb/growth/cross2 command");

  if (!grid->chemostat_flag)
    error->all(FLERR, "fix nufeb/growth/cross2 requires grid_style nufeb/chemostat");
  ...
  if (imetab2 < 0)
    error->all(FLERR, "Can't find substrate(metabolite2) name");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "growth") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg], "metab2_exp") == 0) {
      metab2_exp = utils::numeric(FLERR,arg[iarg+1],true,lmp);
      iarg += 2;
...
void FixGrowthCross2::update_cells()
{
  ...
  for (int i = 0; i < grid->ncells; i++) {
    if (grid->mask[i] & GRID_MASK) {
      // cross_feeder2 growth rate based on substrate(sub) and metabolite1
      double tmp1 = growth * conc[isub][i] / (sub_affinity + conc[isub][i]) * conc[imetab1][i] / (metab1_affinity + conc[imetab1][i]);
      // metabolite2 export-induced growth reduction
      double tmp2 = tmp1 * (-3.4897 * exp(-metab2_exp/0.048) + 3.4092);  // represents the extra sub consumption for metabolite2 production

      // nutrient utilization
      reac[isub][i] -= 1 / yield * (tmp1 + tmp2) * dens[igroup][i];  //total sub consumption
      reac[imetab1][i] -= 1 / yield * tmp1 * dens[igroup][i];
      // metabolite2 export
      reac[imetab2][i] += 0.65 / yield * tmp2 * dens[igroup][i];
    }
  }
}
...
void FixGrowthCross2::update_atoms()
{
  double** x = atom->x;
  ...
  for (int i = 0; i < grid->ncells; i++) {
    ...
    double tmp1 = growth * conc[isub][i] / (sub_affinity + conc[isub][i]) * conc[imetab1][i] / (metab1_affinity + conc[imetab1][i]);
    // metabolite2 export-induced growth reduction
	double tmp3 = tmp1 * (0.141 * exp(-metab2_exp / 0.063) + 0.9);   //about 10% of the growth rate is used for metabolite2 production
